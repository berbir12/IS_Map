-- Store exact browser-provided coordinates for all future submissions.
-- Historical coordinates rounded by an earlier migration cannot be restored.
drop trigger if exists protect_speed_test_location on public.speed_tests;
drop function if exists public.protect_speed_test_location();

comment on column public.speed_tests.latitude is 'Exact browser-provided latitude shared by the contributor.';
comment on column public.speed_tests.longitude is 'Exact browser-provided longitude shared by the contributor.';
...
begin
  ...
  if found then
    ...
    insert into public.speed_tests (
      ...
      p_latitude, p_longitude, p_download_mbps, p_upload_mbps, p_ping_ms, p_connection_type,
